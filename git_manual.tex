...
%Sumaira changes start here

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/fig_chaos.pdf}
    %\caption{Chaos!}
    %\label{fig:myimage}
\end{figure}

%Sumaira changes end here
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/fig_init.pdf}
    %\caption{Chaos!}
    %\label{fig:myimage}
\end{figure}
...
%Sumaira changes start here
This is what the same command will look like for you on you command line interface (CLI)


\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/git_status.png}
    %\caption{Chaos!}
    %\label{fig:myimage}
\end{figure}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=0.8\textwidth]{images/fig_lifecycle.pdf}
%     %\caption{Chaos!}
%     %\label{fig:myimage}
% \end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/fig_three_areas.pdf}
    %\caption{Chaos!}
    %\label{fig:myimage}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/fig_messages.pdf}
    %\caption{Chaos!}
    %\label{fig:myimage}
\end{figure}
%Sumaira changes end here
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/fig_anatomy.pdf}
    %\caption{Chaos!}
    %\label{fig:myimage}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/fig_diff.pdf}
    %\caption{Chaos!}
    %\label{fig:myimage}
\end{figure}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=0.8\textwidth]{images/fig_workflow.pdf}
%     %\caption{Chaos!}
%     %\label{fig:myimage}
% \end{figure}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=0.8\textwidth]{images/fig_timeline.pdf}
%     %\caption{Chaos!}
%     %\label{fig:myimage}
% \end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/fig_conversation.pdf}
    %\caption{Chaos!}
    %\label{fig:myimage}
\end{figure}
...
\section{Looking Ahead: When You Are Not Working Alone}

Remember how we started? Three students were working on the same project:

\begin{itemize}
  \item Ali changed the introduction,
  \item Sara fixed the introduction,
  \item Ahmed added his changes.
\end{itemize}

And we asked three questions: \emph{Which file is correct? What did each person
change? Can we go back to an earlier version?}


% \noindent\rule{\textwidth}{0.4pt}

Everything in this tutorial, \texttt{git commit}, \texttt{git log}, \texttt{git diff},
going back --- answers those questions for \textbf{one person working alone}.
Your project now has a personal time machine.

% \noindent\rule{\textwidth}{0.4pt}


But look closely: Ali, Sara, and Ahmed each built their \emph{own} time machine
on their \emph{own} laptop. That doesn't help them work together yet. If Ali
makes a commit on his laptop, Sara has no way to see it. So one question from the
very beginning is still open:

\begin{center}
\emph{How do several people share one history?}
\end{center}

That is the next chapter, and the tool is \textbf{GitHub}.

GitHub is a place on the internet where a Git repository can live so that everyone
can reach it. Think of it as \textbf{one shared time machine in the cloud} that
Ali, Sara, and Ahmed all connect to.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.85\textwidth]{images/fig_collab.pdf}
  \caption{One shared history: everyone \emph{pushes} their commits up to the same
  GitHub repository and \emph{pulls} everyone else's down.}
  \label{fig:collab}
\end{figure}

Here is the idea, not the full mechanics yet:

\begin{itemize}
  \item Everyone keeps their own copy of the project, with the full history, on
        their own machine, exactly the repository you built in this tutorial.
  \item Everyone connects that copy to the same shared repository on GitHub.
  \item When you finish some work, you send your commits \textbf{up} to GitHub.
        This is called a \textbf{push}.
  \item When you want everyone else's latest work, you bring their commits
        \textbf{down}. This is called a \textbf{pull}.
  \item If two people changed \emph{different} things, Git combines them for you
        automatically (a \textbf{merge}). If they changed the \emph{same} line,
        Git stops and asks a human to decide  (a \textbf{merge conflict}).
\end{itemize}

So the messy folder of \texttt{Science\_Project\_Ali.docx},
\texttt{Science\_Project\_Sara.docx}, and \texttt{Science\_Project\_Ahmed.docx}
simply disappears. There is one project, one shared history, and everyone's
changes flow into it.

\bigskip
\noindent\textbf{A quick look ahead} (you don't need these yet, just meet the words):

\begin{center}
\begin{tabular}{|l|l|}
\hline
\textbf{Term} & \textbf{Meaning (for now)} \\
\hline
GitHub & A website that hosts your Git repository so others can reach it \\
\hline
clone  & Make your own copy of a project that already exists on GitHub \\
\hline
push   & Send your commits up to GitHub \\
\hline
pull   & Bring other people's commits down to your copy \\
\hline
merge  & Combine changes from different people into one history \\
\hline
\end{tabular}
\end{center}

\bigskip
\noindent The one idea to hold on to:

\begin{itemize}
  \item \textbf{Git} (this tutorial) $=$ save and manage your project's history
        \emph{on your own machine}.
  \item \textbf{GitHub} (next) $=$ \emph{share} that history so a whole team can
        work on it together.
\end{itemize}

We'll pick up Ali, Sara, and Ahmed's story there. :-)
